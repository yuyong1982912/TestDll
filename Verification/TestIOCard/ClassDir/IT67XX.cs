...
{	
	...
	public class IT67XX
	{
		...
		public IT67XX(string sPortName,int sBandRate)
		{
			sP=new SerialPort();
		...
		{
			if (sP.IsOpen) {
				Console.WriteLine("67机器已打开");
			} else {
				try {
				sP.Open();
				Console.WriteLine("67机器打开成功");
				} catch (Exception ex) {					
		...
		{			
			...
			bSend[0]=0xaa;bSend[1]=0x00;bSend[2]=0x23;
			...
			if (bReceive[3]==128) {
				Console.WriteLine("67机器设置{0}V电压成功！",iV/1000);
				return true;
			} else {
				return false;
			}
		...
		public bool SetI(double iI)
		{
			...
			for (int i = 5; i < 25; i++) {bSend[i]=0x00;}
			for (int i = 0; i < 25; i++) {				
				i25=i25+Convert.ToInt32(bSend[i]);				
			}			
			s25=Convert.ToString(i25,16).PadLeft(8,'0');//10进制转为16进制字符串
			...
			if (bReceive[3]==128) {
				Console.WriteLine("67机器设置{0}A电流成功！",iI/1000);
				return true;
			} else {
				return false;
			}
